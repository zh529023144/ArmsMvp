 ...
 * ================================================
 */
public class ArmsBasePresenter<M extends IModel, V extends IView> implements IPresenter, LifecycleObserver {
     ...
     */
    @Inject
    public void attach(M model, V rootView) {
        ...
        if (isAttached()){
            if (getView() instanceof LifecycleOwner) {
                ...
                if (mModel!= null && mModel instanceof LifecycleObserver){
                    // 将生命周期代理到model里面
                    ((LifecycleOwner) getView()).getLifecycle().addObserver((LifecycleObserver) mModel);
                }
            }
        }
